...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    //  statement.execute("CREATE TABLE IF NOT EXISTS antenna_percent_agg (date TIMESTAMP, id TEXT, enable DOUBLE PRECISION, disable DOUBLE PRECISION, error DOUBLE PRECISION)")
      ...
      println("Creando la tabla TABLE bytes_hourly(timestamp TIMESTAMP, id TEXT, value BIGINT, type TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS bytes_hourly(timestamp TIMESTAMP, id TEXT, value BIGINT, type TEXT)")

      println("Creando la tabla TABLE user_quota_limit(email TEXT, usage BIGINT, quota BIGINT, timestamp TIMESTAMP)")
      statement.execute("CREATE TABLE IF NOT EXISTS user_quota_limit(email TEXT, usage BIGINT, quota BIGINT, timestamp TIMESTAMP)")

      println("Creando la tabla TABLE user_metadata(id TEXT, name TEXT, email TEXT, quota BIGINT)")
      statement.execute ("CREATE TABLE IF NOT EXISTS user_metadata(id TEXT, name TEXT, email TEXT, quota BIGINT)")

      println("Creando la tabla exceeded (date TIMESTAMP, id TEXT, enable DOUBLE PRECISION, disable DOUBLE PRECISION, error DOUBLE PRECISION)")
      statement.execute("CREATE TABLE IF NOT EXISTS exceed (email TEXT, usage BIGINT, quota BIGINT, timestamp TIMESTAMP, exceeded TEXT)")
